...
def load_card_map(api_url):
    """Busca {arena_id: nome} do mtg_api existente. Em caso de erro de
    rede/endpoint ausente, retorna {} (formatter cai no fallback #grpId)."""
    try:
...
def post_match_start(api_url, state):
    ...
    return _request(f"{api_url}/api/matches", payload, "POST").get("id")


def post_match_end(api_url, match_db_id, state, result):
    ...
    _request(f"{api_url}/api/matches/{match_db_id}", payload, "PATCH")
